...
def _resolve_data_dir() -> Path:
    ...
    for candidate in candidates:
        if (candidate / "hogenom_diams.txt").exists() and (
            candidate / "raxml_diams.txt"
        ).exists():
            return candidate
    raise FileNotFoundError(
        "Could not locate simulation data files. "
        "Expected data/hogenom_diams.txt and data/raxml_diams.txt."
    )
...
def main():
    ...
    parser.add_argument(
        "-n", "--ntrees", type=int, default=50, help="Number of trees to simulate"
    )
    parser.add_argument(
        "-t", "--ntips", type=int, default=20, help="Size of the trees to simulate"
    )
    parser.add_argument(
        "--type",
        type=str,
        help="Simulation methods for the trees: birth-death or uniform",
        default="birth-death",
        choices=["birth-death", "uniform"],
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    hodiams = read_list(DATA_DIR / "hogenom_diams.txt")
    raxdiams = read_list(DATA_DIR / "raxml_diams.txt") * 10
    diams = hodiams + raxdiams
    diams = [item for item in diams if item > 0.02 and item < 15]

    for i in tqdm(range(numtrees), desc="Generating trees"):
        simulateTree(
            i,
            numtips=numtips,
            treeType=treeType,
