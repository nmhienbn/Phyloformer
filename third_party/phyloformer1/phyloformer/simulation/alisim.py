...
def _resolve_alphas_path() -> Path:
    candidates = [
        Path(__file__).resolve().parents[3] / "data" / "hogenom_alphas.txt",
        Path.cwd() / "data" / "hogenom_alphas.txt",
    ]
    for candidate in candidates:
        if candidate.exists():
            return candidate
    raise FileNotFoundError(
        "Could not locate hogenom_alphas.txt. Expected it under data/."
    )


ALPHAS_PATH = _resolve_alphas_path()
...
def main():
    ...
    for treefile in tqdm(treefiles):
        error, attempt = simulate_alignment(
            ...
            args.max_attempts,
            args.processes,
            args.allow_duplicate_sequences,
            args.keep_logfiles,
            alphas,
        )
        if error is not None:
