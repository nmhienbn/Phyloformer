...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Merge PF2 block trees with an external RF-supertree / FastRFS stage, then optional IQ-TREE branch-length refit."
    )
    ...
    parser.add_argument("--fastrfs-bin", required=True, help="Path to FastRFS.")
...
def main() -> None:
    ...
    fastrfs_crashed = False
    if n_sources >= 3:
        fastrfs_bin = Path(args.fastrfs_bin)
        if not fastrfs_bin.is_absolute():
            fastrfs_bin = Path.cwd() / fastrfs_bin

        cmd = [str(fastrfs_bin), "-i", str(source_list), "-o", str(topology_tree)]
        result = subprocess.run(cmd, capture_output=True, text=True)
        if result.returncode != 0:
            fastrfs_crashed = True

    fallback_reason = ""
    if n_sources <= 2 or fastrfs_crashed:
        ...
        fallback_reason = f" fallback=weighted:{'too_few_sources' if n_sources <= 2 else 'fastrfs_crash'}"
    ...
    print(f"[OK] method=fastrfs sources={n_sources}{fallback_reason} tree={output_tree if args.fit_branch_lengths else topology_tree}")
